...
app.post("/add-course", (req, res) => {
    ...
    if (!coursecode || !coursename || !syllabus || !progression) {
        ...
        // Returnera sidan för att lägga till kurs med felmeddelandet
        return res.render("add-course", { error: error, success: false });
    }
    ...
    db.run("INSERT INTO courses (coursecode, coursename, syllabus, progression, posted) VALUES (?, ?, ?, ?, datetime('now', 'localtime'))",
        [coursecode, coursename, syllabus, progression], err => {
            ...
            // Om kursen läggs till framgångsrikt, returnera sidan för att lägga till kurs med framgångsmeddelandet
            res.render("add-course", { error: "", success: "Kursen lades till framgångsrikt." });
        });
});
